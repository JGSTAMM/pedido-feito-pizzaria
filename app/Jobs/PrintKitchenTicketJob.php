...
class PrintKitchenTicketJob implements ShouldQueue, ShouldBeUnique
{
    ...
    {
        return "print:kitchen:{$this->orderId}";
    }
    ...
    ): void
    {
        ...
        if (Cache::has($idempotencyKey)) {
            Log::info('Skipping duplicated kitchen print job for order.', [
        ...
        try {
            ...
            $telemetry->recordSuccess($order, 'kitchen');
        } catch (PrinterNetworkException $exception) {
            $telemetry->recordFailure($order, 'kitchen', $exception->failureType(), $exception->getMessage());
            throw $exception;
    ...
    {
        Log::error('Kitchen print job failed.', [
            'order_id' => $this->orderId,
