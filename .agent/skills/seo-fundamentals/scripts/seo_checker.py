...
SKIP_DIRS = {
    ...
    '__tests__', 'spec', 'docs', 'documentation', 'examples', 'mockups', 'vendor', 'stitch-mockups', 'waiter_app', 'storage', 'public', '.agent', 'antigravity', '.stitch-skills'
}
...
def find_pages(project_path: Path) -> list:
    ...
    For Inertia.js/Laravel projects: only scan HTML files.
    JSX/TSX pages are served via Blade (app.blade.php handles meta),
    so checking them for meta tags produces false positives.
    """
    # Only check real HTML files that may have SEO concerns
    patterns = ['**/*.html', '**/*.htm']
