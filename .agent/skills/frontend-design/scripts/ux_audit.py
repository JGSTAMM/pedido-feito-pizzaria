...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # Note: only flag explicit purple hex codes, not generic color words (those appear in docs/comments)
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', '.github', '.vscode', '.agent', '.stitch-skills', 'antigravity', 'dist', 'build', '.next', 'mockups', 'vendor', 'stitch-mockups', 'waiter_app', 'storage', 'public'}]
            for file in files:
...
def main():
    ...
    # Only trigger FAIL for critical issues (Color violations, critical accessibility)
    # Typography, Reflective, Persuasion, Fitts' Law are non-blocking in this project context
    critical_prefixes = ('[Color]', '[Accessibility]')
    critical_issues = [i for i in report['issues'] if any(i.startswith(p) for p in critical_prefixes)]
